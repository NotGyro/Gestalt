...
use crate::resource::image::{ID_MISSING_TEXTURE, ID_PENDING_TEXTURE, ImageProvider, InternalImage};
use glam::Vec2;
...
impl TileAtlasLayout {
    ...
    pub fn new(tile_size: u32,
            initial_width: u32, initial_height: u32, 
            max_tiles: Option<usize>) -> Self {
    ...
    pub fn calc_resolution(&self) -> (u32,u32) {
        ( (self.atlas_width * self.tile_size), ( height_from_num_tiles(self.tiles.len(), self.atlas_width) * self.tile_size) )
    }
    ...
    pub fn get_or_make_index_for_texture(&mut self, resource: &ResourceId) -> Result<usize,TileAtlasError>  { 
        match self.get_index_for_texture(resource) { 
            Some(idx) => Ok(idx),
            None => {
                let idx = self.tiles.len();
                if idx >= self.max_tiles { 
                    return Err(TileAtlasError::OverMax(resource_debug!(resource), self.max_tiles));
                }
    ...
    /// higher_x and higher_y, when set, give you the higher-x and higher-y coordinates on the rectangle,
    /// respectively. If both are false, you get the top-left corner. 
    pub fn get_or_make_uv_for_texture(&mut self, resource: &ResourceId, higher_x: bool, higher_y: bool) -> Result<Vec2,TileAtlasError> {
        let idx = self.get_or_make_index_for_texture(resource)?;
        Ok( 
            self.get_uv_for_index(idx, higher_x, higher_y)
        )
    }
...
pub fn build_tile_atlas<TextureSource: ImageProvider>(layout: &TileAtlasLayout, texture_source: &mut TextureSource) -> Result<InternalImage, TileAtlasError> {
    ...
    for (tile_index, resource_tile) in layout.tiles.iter().enumerate() { 
        ...
        if !((texture_to_use.width() == layout.tile_size) && (texture_to_use.height() == layout.tile_size)) {
            error!(
        ...
        let mut sub_image = atlas.sub_image(x * layout.tile_size, y * layout.tile_size, layout.tile_size, layout.tile_size);
