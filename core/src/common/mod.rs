...
impl Version {
    ...
    pub fn parse(value: &str) -> std::result::Result<Self, ParseVersionError> { 
        ...
        fn number_from_field(field: &str, original_string: String) -> Result<u32, ParseVersionError> {
            let big_number = u128::from_str_radix(field, 10)
...
pub mod version_string { 
    ...
    pub fn serialize<S>(val: &Version, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
        serializer.serialize_str(val.to_string().as_str())
    }

    pub fn deserialize<'de, D>(deserializer: D) -> Result<Version, D::Error> where D: Deserializer<'de> {
