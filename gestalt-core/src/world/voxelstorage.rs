...
pub mod voxel_bulk_ops {
	...
	where
		AE: std::error::Error + Sized,
		BE: std::error::Error + Sized,
	{
		ErrorA(AE),
		ErrorB(BE),
	}
	...
	pub trait VoxelNeighborhood: Clone + Sized {
		...
		fn new(center: Self::SourceVoxel, sides: SidesArray<Self::SourceVoxel>) -> Self;
