...
pub enum TileSpaceError {
	...
	ChunkError(#[from] <chunk::Chunk<TileId> as VoxelStorage<TileId, u8>>::Error),
	#[error(
	...
	fn kind(&self) -> VoxelErrorCategory {
		match self {
			...
			TileSpaceError::ChunkError(e) => match e {
				super::voxelarray::VoxelArrayError::OutOfBounds(_) => {
					VoxelErrorCategory::OutOfBounds
				}
				super::voxelarray::VoxelArrayError::NoPaletteEntry(_) => {
					VoxelErrorCategory::PaletteIssue
				}
			},
...
impl TileSpace {
	...
	pub fn ingest_loaded_chunk(
		&mut self,
		pos: ChunkPos,
		chunk: chunk::Chunk<TileId>,
	) -> Result<(), TileSpaceError> {
...
pub fn world_to_chunk_pos(v: &TilePos) -> ChunkPos {
	vpos!(
		v.x >> CHUNK_EXP as ChunkCoord,
		v.y >> CHUNK_EXP as ChunkCoord,
		v.z >> CHUNK_EXP as ChunkCoord
	)
}
...
pub fn chunk_to_world_pos(v: &ChunkPos) -> TilePos {
	vpos!(
		v.x * CHUNK_SIZE as TileCoord,
		v.y * CHUNK_SIZE as TileCoord,
		v.z * CHUNK_SIZE as TileCoord
	)
}
...
impl VoxelStorage<TileId, TileCoord> for TileSpace {
	...
	fn get(&self, pos: TilePos) -> Result<&TileId, TileSpaceError> {
		...
		match self.chunks.get(&vpos!(chx, chy, chz)) {
			Some(chunk) => Ok(chunk.get(vpos!(x as u8, y as u8, z as u8))?),
...
impl VoxelSpace<TileId> for TileSpace {
	...
	fn borrow_chunk(
		...
		self.chunks
			.get(chunk)
			.ok_or(TileSpaceError::NotYetLoaded(*chunk))
	}
	...
	fn get_loaded_chunk_cells(&self) -> Vec<&ChunkPos> {
