...
use crate::{common::identity::NodeIdentity, message::{MpscSender, MpscReceiver, MpscChannel, SenderChannel, MessageReceiverAsync, MessageReceiver}};

use super::{ResourcePath, ResourceError, ResourcePoll, retrieval::{ResourceFetchResponse, ResourceFetch}, channels::RESOURCE_FETCH, ResourceRetrievalError};
...
pub trait ResourceProvider<T> { 
	...
	fn request_one(&mut self, resource: ResourcePath, expected_source: NodeIdentity) 
			-> Option<Result<T, ResourceError<Self::ParseError>>> {
		self.request_batch(vec![resource], expected_source).pop()
			.map(|value| 
				value.map(|result| result.1)
			)
	}
	...
	fn recv_wait(&mut self) -> impl Future<Output = Result<(ResourcePath, T), ResourceError<Self::ParseError>>> + '_;
		...
		while !next.is_none() { 
			match next {
				...
				ResourcePoll::None => unreachable!(
					"Unreachable due to \"while next != ResourcePoll::None\" above."
				),
			}
...
impl RawResourceProvider {
	...
	fn request_inner(&self, resources: Vec<ResourcePath>, expected_source: NodeIdentity, 
		...
		self.fetch_sender.blocking_send(ResourceFetch {
			resources: resources.iter().map(|value|{
				match value {
	...
	async fn recv_wait_inner(&mut self) -> Result<(ResourcePath, Arc<Vec<u8>>), ResourceError<ResourceRetrievalError>> { 
...
impl ResourceProvider<Arc<Vec<u8>>> for RawResourceProvider {
    ...
    fn request_batch(&mut self, resources: Vec<ResourcePath>, expected_source: NodeIdentity) 
			-> Vec<Result<(ResourcePath, Arc<Vec<u8>>), 
                    ResourceError<ResourceRetrievalError>>
                > {
    ...
    fn recv_wait(&mut self) -> impl Future<Output = Result<(ResourcePath, Arc<Vec<u8>>), ResourceError<ResourceRetrievalError>>> + '_ {
