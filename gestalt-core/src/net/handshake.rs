...
pub(in super) fn noise_protocol_dir(protocol_store_dir: &PathBuf) -> PathBuf {
...
pub async fn load_noise_local_keys(noise_dir: PathBuf, our_ident: NodeIdentity) -> Result<snow::Keypair, HandshakeError> {
    ...
    let keypair = if path.exists() {
        ...
        file.read_exact(&mut private).await
            .map_err( HandshakeError::ProtocolStoreIoError )?;
        file.read_exact(&mut public).await
            .map_err( HandshakeError::ProtocolStoreIoError )?;
...
async fn await_key_approver_response(identity: NodeIdentity, mut approver: NewProtocolKeyApprover) -> Result<bool, HandshakeError> {
...
async fn load_validate_noise_peer_key(
        ...
        mismatch_approver: NewProtocolKeyApprover) -> Result<(), HandshakeError> {
    async fn ld_keys(peer_filepath: PathBuf) -> Result<PeerKeyFile, HandshakeError> { 
        ...
        let mut buf = Vec::new();
        let read_amt = file.read_to_end(&mut buf).await
            .map_err( HandshakeError::ProtocolStoreIoError )?;
        buf.truncate(read_amt);

        // from_read() doesn't work here because we're using async i/o instead. 
        rmp_serde::from_slice(&buf)
            .map_err( HandshakeError::ProtocolStoreDecodeError )
    }
    async fn create_new_peer_key_file(peer_filepath: PathBuf, peer_identity: NodeIdentity, received_noise_key: [u8;32]) -> Result<(), HandshakeError> { 
    ...
    async fn write_changed_peer_key_file(peer_filepath: PathBuf, keys: PeerKeyFile) -> Result<(), HandshakeError> {
        // Convert to msgpack
        let writebuf = rmp_serde::to_vec_named(&keys)
            .map_err(HandshakeError::ProtocolStoreEncodeError)?;
        ...
        tokio::fs::rename(&peer_filepath, &bkfile).await
            .map_err( HandshakeError::ProtocolStoreIoError )?;
        tokio::fs::rename(&temp_filepath, &peer_filepath).await
            .map_err( HandshakeError::ProtocolStoreIoError )?;
        tokio::fs::remove_file(bkfile).await
            .map_err( HandshakeError::ProtocolStoreIoError )?;
        
        Ok(())
    }
    let peer_filepath = peer_dir(&noise_dir)
        .join(PathBuf::from(peer_identity.to_base64()));
                ...
                match key_struct.keys.contains(&received_noise_key) { 
                    ...
                    false => {
                        ...
                        match await_key_approver_response(peer_identity.clone(), mismatch_approver).await? {
                            true => { 
                                info!("New peer key accepted for {}, adding to their list of recognized keys.", peer_identity.to_base64()); 
            ...
            Err(e) => { 
                ...
                match await_key_approver_response(peer_identity.clone(), mismatch_approver).await? {
                    true => {
                        ...
                        create_new_peer_key_file(peer_filepath.clone(), peer_identity.clone(), received_noise_key.clone()).await
                    },
                    false => Err(HandshakeError::IdentityChanged(peer_identity.to_base64())),
                }
            }
...
pub fn receive_initial(
        ...
        input: HandshakeStepMessage) -> Result<(snow::HandshakeState, HandshakeStepMessage), HandshakeError> { 
...
pub async fn initiator_reply(
    ...
    if input.handshake_step == 2 {
        ...
        load_validate_noise_peer_key(noise_dir, 
            peer_gestalt_identity.clone(), 
            protocol_key,
            report_mismatch,
            mismatch_approver).await?;
...
pub async fn receive_last_noise(mut state: snow::HandshakeState, 
        input: HandshakeStepMessage, 
        noise_dir: PathBuf,
        our_gestalt_identity: NodeIdentity,
        report_mismatch: NewProtocolKeyReporter,
...
pub fn initiator_sign_buf(state: snow::StatelessTransportState, input: HandshakeStepMessage, their_key: &NodeIdentity, our_keys: &IdentityKeyPair, handshake_hash: &Vec<u8>) 
    ...
    if input.handshake_step == 4 {
        ...
        let decoded_session_id = base64::decode(&challenge.session_id)
            .map_err(|_| HandshakeError::BadChallengeHeader)?;
...
pub fn responder_sign(state: snow::StatelessTransportState, input: HandshakeStepMessage, our_keys: &IdentityKeyPair, peer_identity: &NodeIdentity, our_challenge: String, handshake_hash: &Vec<u8>) 
...
pub fn initiator_final(state: snow::StatelessTransportState, input: HandshakeStepMessage, peer_identity: &NodeIdentity, our_challenge: String) 
...
impl HandshakeIntitiatorState {
    ...
    pub fn get_step(&self) -> HandshakeIntitiatorStep { 
        match self {
            ...
            HandshakeIntitiatorState::SentFifthAwaitSixth(_, _, _, _, _) => HandshakeIntitiatorStep::SentFifthAwaitSixth,
...
impl HandshakeInitiator {
    pub fn new(noise_dir: PathBuf,
            local_noise_keys: snow::Keypair, 
            local_gestalt_keys: IdentityKeyPair,
            report_mismatch: NewProtocolKeyReporter, 
            mismatch_approver: NewProtocolKeyApprover ) -> Self { 
        HandshakeInitiator { 
    ...
    pub async fn advance(&mut self, incoming: HandshakeStepMessage)
        ...
        match self.last_state.take().unwrap() {
            ...
            HandshakeIntitiatorState::SentThirdAwaitFourth(state, peer_id, sid) => { 
                let (transport, message, nonce, seq) = initiator_sign_buf(state, incoming, &peer_id, &self.local_gestalt_keys, &sid)?;
                self.last_state = Some(HandshakeIntitiatorState::SentFifthAwaitSixth(transport, nonce, seq, peer_id, sid));
                Ok(HandshakeNext::SendMessage(message))
            },
            ...
            HandshakeIntitiatorState::Done(_, _, _, _) => {
...
impl HandshakeReceiver {
    ...
    pub async fn advance(&mut self, incoming: HandshakeStepMessage)
        ...
        match self.last_state.take().unwrap() {
            HandshakeReceiverState::Init => { 
                let (state, message) = receive_initial(clone_keypair(&self.local_noise_keys), self.local_gestalt_keys.public.clone(), incoming)?;
...
#[cfg(test)]
pub async fn approver_no_mismatch(mut receiver: BroadcastReceiver<NodeIdentity>, _sender: BroadcastSender<(NodeIdentity, bool)>) {
    match receiver.recv_wait().await { 
        Ok(_) => panic!("New connection with an unrecognized key - this test should never evaluate the mismatch code path."),
...
#[cfg(test)]
mod test {
    use crate::message::{BroadcastChannel, SenderChannel, ReceiverChannel};
    use super::*;
    ...
    async fn handshake_test() {
        ...
        let (alice_state, message_2) = receive_initial(clone_keypair(&alice_noise_keys), alice_gestalt_keys.public.clone(), message_1).unwrap();
        let (bob_transport, 
            message_3, 
        ...
        let (alice_transport, 
            ...
            = receive_last_noise(
                ...
                mismatch_report_channel.sender_subscribe(), 
                mismatch_approve_channel.receiver_subscribe()).await.unwrap();
        let (bob_transport, message_5, bob_nonce, _bob_seq) = initiator_sign_buf(bob_transport, message_4, &bob_copy_alice_identity, &bob_gestalt_keys, &bob_session_id).unwrap();
        let (alice_transport, message_6, _alice_seq) = responder_sign(alice_transport, message_5, &alice_gestalt_keys, &alice_copy_bob_identity, alice_nonce, &alice_session_id).unwrap();
        let (bob_transport, _bob_seq) = initiator_final(bob_transport, message_6, &bob_copy_alice_identity, bob_nonce).unwrap();
    ...
    async fn handshake_state_machine_test() {
        ...
        let mut initiator = HandshakeInitiator::new(PathBuf::from(noise_dir.path()), bob_noise_keys, bob_gestalt_keys, mismatch_report_channel.sender_subscribe(), mismatch_approve_channel.receiver_subscribe());
        let mut receiver = HandshakeReceiver::new(PathBuf::from(noise_dir.path()), alice_noise_keys, alice_gestalt_keys, mismatch_report_channel.sender_subscribe(), mismatch_approve_channel.receiver_subscribe());
