...
impl OuterEnvelope { 
    ...
    pub fn encode(&self, send_buf: &mut [u8]) -> Result<usize, OuterEnvelopeError> {
        ...
        if send_buf.len() < SESSION_ID_LEN + COUNTER_LEN + 1 { 
            return Err(OuterEnvelopeError::NotEnoughForHeader(send_buf.len(), SESSION_ID_LEN + COUNTER_LEN + 1));
        }
    ...
    pub fn decode_packet(recv_buf: &[u8], peer_address: SocketAddr ) -> Result<(OuterEnvelope, usize), OuterEnvelopeError> {
        ...
        if recv_buf.len() < SESSION_ID_LEN + COUNTER_LEN + 1 { 
            return Err(OuterEnvelopeError::NotEnoughForHeader(recv_buf.len(), SESSION_ID_LEN + COUNTER_LEN + 1))
        }
