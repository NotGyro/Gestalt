...
impl PreProtocolReceiver {
	...
	pub fn is_handshake_done(&self) -> bool {
		match &self.state {
			PreProtocolReceiverState::QueryAnswerer => false,
			PreProtocolReceiverState::Handshake(receiver) => receiver.is_done(),
		}
	}
	...
	pub async fn receive_and_reply(
		&mut self,
		incoming: PreProtocolQuery,
	) -> Result<PreProtocolOutput, PreProtocolError> {
		Ok(match incoming {
			PreProtocolQuery::SupportedProtocols => {
				PreProtocolOutput::Reply(PreProtocolReply::SupportedProtocols(SupportedProtocols {
					supported_protocols: SUPPORTED_PROTOCOL_SET.clone(),
				}))
			}
			PreProtocolQuery::RequestServerStatus => {
				PreProtocolOutput::Reply(PreProtocolReply::Status(*SERVER_STATUS.clone().lock()))
			}
			PreProtocolQuery::StartHandshake(start_handshake) => {
				...
				if !self.state.is_in_handshake() {
					...
					PreProtocolOutput::Reply(PreProtocolReply::Err(format!(
						"Handshake error: {:?}",
						PreProtocolError::HandshakeAlreadyStarted
					)))
				}
			}
...
pub async fn preprotocol_receiver_session(
	our_identity: IdentityKeyPair,
			...
			match serde_json::from_str::<PreProtocolQuery>(&msg) {
				Ok(query) => {
					match receiver.receive_and_reply(query).await {
							...
							match out {
								PreProtocolOutput::Reply(to_send) => {
									...
									match receiver.is_handshake_done() {
										true => {
											let (transport, seq, peer_identity, session_id) =
												receiver.complete_handshake().unwrap();
											info!(
											...
											match receiver.peer_role {
												...
												_ => {
													...
													let reply = PreProtocolReply::Err(format!(
														"{:?}",
						...
						Err(e) => {
							error!(
								"Preprotocol loop error communicating with {}: {:?}",
				...
				Err(e) => {
					error!(
						"Error parsing PreProtocolQuery from json received from {}: {:?}",
						stream.peer_addr().unwrap(),
						e
					);
...
pub async fn preprotocol_connect_inner(
	...
	let mut handshake_initiator = HandshakeInitiator::new(
		noise_dir,
		load_noise_local_keys(protocol_dir.clone(), our_identity.public).await?,
		our_identity,
		report_mismatch,
		mismatch_approver,
	);
	...
	trace!(
		"Connection to {:?} successful and our cryptographic counter is {}",
		server_identity.to_base64(),
		counter
	);
