...
impl<T> Clone for BroadcastChannel<T> where T: Message {
    fn clone(&self) -> Self {
        Self { sender: self.sender.clone(), retained_receiver: self.retained_receiver.clone() }
    }
}
...
impl<T, D, C> DomainMultiChannel<T, D, C>  where T: Message, D: ChannelDomain, C: SenderChannel<T> + ChannelInit  {
    ...
    pub fn sender_subscribe(&self, domain: &D) -> Result<C::Sender, DomainSubscribeErr<D>> {
        Ok(self.channels.lock().get_mut(domain)
    ...
    pub fn add_domain(&self, domain: &D) {
        self.channels.lock().entry(domain.clone()).or_insert(C::new(self.capacity));
    }
...
pub mod test {
    ...
    async fn domain_channels() {
        ...
        let other_channel_sender = TEST_DOMAIN_CHANNEL.sender_subscribe(&some_other_player_identity).unwrap();
        let mut other_channel_receiver = TEST_DOMAIN_CHANNEL.receiver_subscribe(&some_other_player_identity).unwrap();
