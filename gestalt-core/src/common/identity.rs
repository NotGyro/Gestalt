...
use pkcs8::{DecodePrivateKey, DecodePublicKey, EncodePrivateKey, EncodePublicKey, der::Document, AlgorithmIdentifier};
...
pub fn generate_local_keys(passphrase: Option<String>) -> Result<IdentityKeyPair, Box<dyn std::error::Error>> {
    ...
    if priv_key_path.exists() {
        return Err(
            Box::new(
                KeyPairLoadError::PrivKeyExists
            )
        );
    }
    ...
    let priv_key_file_string = match passphrase {
        Some(pass) => {
            keypair_bytes.to_pkcs8_encrypted_pem(rng, pass, pkcs8::LineEnding::default())
        },
        None => {
            keypair_bytes.to_pkcs8_pem(pkcs8::LineEnding::default())
        },
    }?;
    ...
    let pub_key_document = pkcs8::PublicKeyDocument::try_from(pkcs8::SubjectPublicKeyInfo {
        algorithm: AlgorithmIdentifier { oid: ed25519::pkcs8::ALGORITHM_OID, parameters: None },
...
pub fn load_local_identity_keys(passphrase: Option<String>) -> Result<IdentityKeyPair, Box<dyn std::error::Error>> {
    ...
    if public_key_info.subject_public_key.len() !=  PUBLIC_KEY_LENGTH { 
        return Err(Box::new(KeyPairLoadError::WrongLengthPublic(public_key_info.subject_public_key.len())));
    }
    public_key_bytes.copy_from_slice(&public_key_info.subject_public_key[0..PUBLIC_KEY_LENGTH]);

    public_key_info.algorithm.assert_algorithm_oid(ed25519::pkcs8::ALGORITHM_OID)?;
