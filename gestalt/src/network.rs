...
impl ClientNet {
    ...
    pub fn connect(&mut self, server_addr: SocketAddr) -> Result<(), Box<dyn Error>> {
        ...
        loop {
                ...
                Some(event) => {
                    match event {
                        SocketEvent::Packet(packet) => {
                            ...
                            info!(Network, "Got another handshake message from {:?}! \n Its contents are: {:?}", packet.addr(), message);
                            match message {
                                HandshakeMessage::Prelude(their_prelude) => {
                                    info!(Network, "Received server's handshake, sending our signature.");
                                ...
                                HandshakeMessage::Signature(their_sig) => {
                                    info!(Network, "Got a signature from the server.");
                                ...
                                HandshakeMessage::Response(response) => match response {
                                    ...
                                    HandshakeResponse::Accepted => {
                                        info!(Network, "Server accepted our connection!");
            ...
            if !server_is_valid {
                ...
                if let Some(their_prelude) = server_prelude {
                    info!(Network, "We do have a server prelude");
                    if let Some(their_sig) = server_sig {
                        info!(Network, "We do have a server signature");
                        if their_sig.verify(their_prelude.public_key, &our_prelude.please_sign.to_vec(), their_prelude.version) {
                            ...
                            info!(Network, "Server's sig verified data! Setting server_is_valid = true");
                            ...
                            socket.send(Packet::reliable_unordered(server_addr, serialize(&response)?))?;
                            error!(Network, "Server's sig did not verify data.");
        ...
        if server_is_valid {
            info!(Network, "Connection to server completed!");
                ...
                match serv.1.receiver.try_recv() {
                    Ok(event) => match event {
                        SocketEvent::Packet(packet) => {
                            ...
                            info!(Network, "Received {:?} from {:?}", text, ip);
                        ...
                        SocketEvent::Timeout(server_address) => {
                            info!(Network, "Server timed out: {:?}", server_address);
                    ...
                    Err(e) => {
                        error!(Network, "Other TryRecvError: {:?}", e);
...
impl ServerNet {
    pub fn new(identity: &SelfIdentity, our_address: SocketAddr) -> Result<Self, Box<dyn Error>> {
        ...
        info!(Network, "Initiating server on {:?}", our_address);
        Ok(ServerNet {
            keys: identity.clone(),
            clients: HashMap::new(),
    ...
    fn process_packet(&mut self, packet: &Packet) -> Result<(), Box<dyn Error>> {
        if IP_BANS.lock().contains(&packet.addr().ip()) {
            ...
            info!(Network, "A client attempted to connect from {}, but that IP is banned.", packet.addr().ip());
        }
        //They have been recorded as "Connecting" but have not yet sent a package.
            ...
            if let HandshakeMessage::Prelude(prelude) = handshake_message {
                info!(Network, "Got a handshake message from client!");
                ...
                info!(Network, "Putting client {:?} into handshake stage.", packet.addr());
            }
        }
            ...
            if is_done {
                info!(Network, "{:?} is now authorized.", packet.addr());
            ...
            warn!(Network, "Abnormal packet from {:?}", packet.addr());
        }
        Ok(())
    }
            ...
            match self.incoming.try_recv() {
                Ok(event) => match event {
                    SocketEvent::Connect(client_address) => {
                        info!(Network, "Incoming connection from {:?}!", client_address);
                        ...
                        if (!self.clients.contains_key(&client_address)
                            ...
                            info!(Network, "Queueing client {:?} for authorization.", client_address);
                    ...
                    SocketEvent::Packet(packet) => {
                        match self.process_packet(&packet) {
                            ...
                            Err(e) => {
                                error!(Network, "Got an error while processing a packet from {:?}. \n The error is: {}" , packet.addr(), e);
                    ...
                    SocketEvent::Timeout(client_address) => {
                        info!(Network, "Client timed out: {:?}", client_address);
