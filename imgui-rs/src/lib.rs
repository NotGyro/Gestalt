...
pub fn get_id_str(s: &ImStr) -> Id<'static> {
    let i;
    unsafe { i = sys::igGetIDStr(s.as_ptr()) as i32; }
    Id::Int(i)
}

pub fn dock_space(id: Id, size: [f32; 2], flags: ImGuiDockNodeFlags) {
    if let Id::Int(i) = id {
        unsafe { sys::igDockSpace(i as u32, sys::ImVec2::new(size[0], size[1]), flags.bits() as i32, 0 as *const c_void as *const ImGuiWindowClass) }
    }
}
